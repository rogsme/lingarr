...
public class MistralService : BaseLanguageService, ITranslationService, IBatchTranslationService, IProofreadService
{
    private readonly string? _endpoint = "https://api.mistral.ai/v1";
    ...
    private async Task InitializeAsync(string sourceLanguage, string targetLanguage)
    {
        ...
        try
        {
            ...
            _model = settings[SettingKeys.Translation.Mistral.Model];
            _apiKey = await _settings.GetEncryptedSetting(SettingKeys.Translation.Mistral.ApiKey);
            _requestTemplate = !string.IsNullOrEmpty(settings[SettingKeys.Translation.Mistral.RequestTemplate])
                ? settings[SettingKeys.Translation.Mistral.RequestTemplate]
                : _requestTemplateService.GetDefaultTemplate(SettingKeys.Translation.Mistral.RequestTemplate);

            if (string.IsNullOrEmpty(_model) || string.IsNullOrEmpty(_apiKey))
            {
                throw new InvalidOperationException("Mistral API key or model is not configured.");
            }
        ...
        CancellationToken cancellationToken)
    {
        ...
        for (var attempt = 1; attempt <= _maxRetries; attempt++)
        {
            try
            {
                ...
                return await CompleteWithMistralApi(replacements, linked.Token);
            }
            catch (HttpRequestException ex) when (ex.StatusCode is HttpStatusCode.TooManyRequests or HttpStatusCode.ServiceUnavailable)
            {
                ...
                _logger.LogWarning(
                    ...
                    "Mistral", ex.StatusCode, delay, attempt, _maxRetries);
            }
            ...
            {
                _logger.LogError(ex, "Error occurred during Mistral translation");
                throw new TranslationException("Failed to translate using Mistral", ex);
            }
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        for (var attempt = 1; attempt <= _maxRetries; attempt++)
        {
            ...
            {
                _logger.LogError(ex, "Error occurred during Mistral proofread");
                throw new TranslationException("Failed to proofread using Mistral", ex);
            }
        }
    ...
    private async Task<string> CompleteWithMistralApi(
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!response.IsSuccessStatusCode)
        {
            if (response.StatusCode is HttpStatusCode.TooManyRequests or HttpStatusCode.ServiceUnavailable)
            {
                throw new HttpRequestException(
                    $"Mistral returned {response.StatusCode}", null, response.StatusCode);
            }

            var responseContent = await response.Content.ReadAsStringAsync(cancellationToken);
            _logger.LogError(
                "Mistral API request failed with status {StatusCode}: {ResponseContent}",
                response.StatusCode, responseContent);
            throw new TranslationException(
                $"Mistral API request failed with status {response.StatusCode}: {responseContent}");
        }
        ...
        {
            throw new TranslationException("No completion choices returned from Mistral");
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        for (var attempt = 1; attempt <= _maxRetries; attempt++)
        {
            try
            {
                return await TranslateBatchWithMistralApi(subtitleBatch, linked.Token);
            }
            catch (HttpRequestException ex) when (ex.StatusCode is HttpStatusCode.TooManyRequests or HttpStatusCode.ServiceUnavailable)
            {
                if (attempt == _maxRetries)
    ...
    private async Task<Dictionary<int, string>> TranslateBatchWithMistralApi(
        List<BatchSubtitleItem> subtitleBatch,
        CancellationToken cancellationToken)
    {
        var requestUrl = $"{_endpoint}/chat/completions";
        ...
        if (!response.IsSuccessStatusCode)
        {
            ...
            {
                throw new HttpRequestException(
                    $"Batch translation using Mistral API failed with {response.StatusCode}.",
                    null, response.StatusCode);
            }

            var responseContent = await response.Content.ReadAsStringAsync(cancellationToken);
            _logger.LogError(
                "Mistral batch API request failed with status {StatusCode}: {ResponseContent}",
                response.StatusCode, responseContent);
            throw new TranslationException(
                $"Mistral batch API request failed with status {response.StatusCode}: {responseContent}");
        }
    ...
    public override async Task<ModelsResponse> GetModels()
    {
        ...
        try
        {
            ...
            if (modelsResponse?.Data == null)
            {
                return new ModelsResponse
                {
                    Message = "No models data returned from Mistral API."
                };
            }
        ...
        {
            _logger.LogError(ex, "HTTP error fetching models from Mistral API");
            return new ModelsResponse
            {
                Message = $"HTTP error fetching models from Mistral API: {ex.Message}"
            };
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error fetching models from Mistral API");
            return new ModelsResponse
            {
                Message = $"Error fetching models from Mistral API: {ex.Message}"
            };
        }
    }
}
